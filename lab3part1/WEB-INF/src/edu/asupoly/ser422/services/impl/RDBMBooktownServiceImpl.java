...
public class RDBMBooktownServiceImpl implements BooktownService {
	...
	public List<Author> getAuthors() {
		...
		try {
			...
			rs = stmt.executeQuery("Select max(id) from Authors");
			while (rs.next()) {
				__id = rs.getInt(1)+1;
			}
...
//			__id = rval.size() + 1;
	...
	public int createAuthor(String lname, String fname) {
		...
		try {
			...
			conn = DriverManager.getConnection(__jdbcUrl, __jdbcUser, __jdbcPasswd);
			stmt = conn.createStatement();
			System.out.println("id insering: "+__id);
	...
	static {
		try {
			...
			dbProperties.load(RDBMBooktownServiceImpl.class.getResourceAsStream("rdbm.properties"));
			...
			System.out.println("couldnt read properties");
			t.printStackTrace();
		} finally {
		}
	}
