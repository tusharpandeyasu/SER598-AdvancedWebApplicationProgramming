...
fs.open('./codersArray.txt', "w", function (err, fd) {
	if (err){
		console.log("Failed to open codersArray.txt File.");
    }
...
app.get('/', function(req, res) {	 
	...
	if (!req.cookies.firstname && !req.session.visited){	  
		...
		fs.readFile('./codersArray.txt', options, function(err, data){
			if (err){
				console.log("Failed to open codersArray File.");
			} else {
				console.log("codersArray Loaded.");
				var codersArray = JSON.parse(data);
	...
	if (req.body.firstname === '' || req.body.lastname === '') {
		...
		fs.readFile('./codersArray.txt', options, function(err, data){
			if (err){
	    		...
	    		var codersArrayTxt = JSON.stringify(codersArray);
	    	    var options = {encoding:'utf8', flag:'w'};
	    	    fs.writeFile('./codersArray.txt', codersArrayTxt, options, function(err){
	    	      if (err){
	    	        console.log("codersArray Write Failed.");
	    	      } else {
	    	        console.log("codersArray Saved.");
...
//Handle 405 for GET Method
app.post('/', function(req, res) {
	res.status(405);
	   res.render('405.jade', {title:'Method Not Allowed'});
});

//Handle 405 for POST Method
